...
Space_Init:
	Space_Str := "abcdefghijklmnopqrstuvwxyz1234567890"
	Space_arr := StrSplit(Space_Str)
	...
	Hotkey, If, % fn
	Loop, % Space_arr.length() {
		Hotkey, % Space_arr[A_Index], Get_Key
	}
return
...
Get_Key:
var := var . A_ThisHotkey
;ToolTip, %var%
return
/*
;---------------------------------------------------------------------------------------------------------------------------
;这部分写法将会导致按下空格时，CPU占用率提高5%左右，所以不太好，加上sleep虽然CPU占用率降低了，又会影响捕捉效果，依然不满足要求。
;---------------------------------------------------------------------------------------------------------------------------
~Space::
var := ""
Loop{
	if not GetKeyState("Space", "P")
		return
	Index++
	Index := Index > 36 ? 1 : Index
	State := Space_arr[Index]
	if GetKeyState(Space_arr[Index], "P") {
		if (%State% = 1) {
			var := var . State
			ToolTip, %var%
		}
...
Suspend_s() {	; 此函数用于暂停 空格 触发的 启动应用 热键
	global judge_s
	return judge_s
}
...
Suspend_GUI(TIPS) {
	Gui, P_S:Destroy
	Gui, P_S:+LastFound +ToolWindow -Caption +AlwaysOnTop +Border
	Gui, P_S:color, Gray
	Gui, P_S:Font, S20
	WinSet, TransParent, 255
	WinSet, ExStyle, +0x20
	Gui, P_S:Add, Text, x0 y0 cBlue, 热键服务 已&%TIPS%
	Gui, P_S:Show, w205 h28
	SetTimer, Over, 2000
	return
